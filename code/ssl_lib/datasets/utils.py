...
def get_svhn(root):
    ...
    train_data = SVHN(root = root, split = 'train', download = False)
    test_data = SVHN(root = root, split = 'test', download = False)
    train_data = {"images": train_data.data.astype(np.uint8).reshape(-1,32,32,3),
                  "labels": np.asarray(train_data.labels)}
    test_data = {"images": test_data.data.astype(np.uint8).reshape(-1,32,32,3), 
                 "labels": np.asarray(test_data.labels)}
    return train_data, test_data
